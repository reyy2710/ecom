...
def checkout(request):
    ...
    if request.user.is_authenticated:
        ...
        # Shipping Form
        shipping_form = ShippingForm(request.POST or None, instance=shipping_user) #instance helps us put in the already saved info
...
def order_detail(request, order_id):
    ...
    for item in order.items.all():
        subtotal_items.append({
            ...
            'price': item.price,
            'subtotal': item.quantity * item.price  # Calculate subtotal here
        })
...
def process_order(request):
    if request.POST:
        ...
        messages.success(request, "Order Placed! A confirmation email has been sent.")
        return redirect('home')
